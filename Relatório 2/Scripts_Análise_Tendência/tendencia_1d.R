...
#leitura dos dados
dados_maximo_diario <- read_excel("C:/Users/bruno/OneDrive/Área de Trabalho/Bruno/UnB/Tendências/Código_original_Saulo/Teste_resultados/indices_precipitacao_anuais.xlsx") %>%
  mutate(
    gauge_code = as.character(gauge_code),
    year = as.integer(year)
  )

max_ano <- dados_maximo_diario %>%
  group_by(year) %>%
  summarise(max_chuva = max(chuva_max, na.rm = TRUE))

#ler shp das regioes hidrográficas
...
#preparar
regioes_hidrograficas <- regioes_hidrograficas %>%
  select(
    RHI_SG,
    RHI_CD,
    RHI_NM,
...
dados_maximo_diario <- dados_maximo_diario %>%
  left_join(
    estacoes_rh %>%
      st_drop_geometry() %>%
      select(
...
#plotar máximos
graf_chuva<-ggplot(max_ano, aes(x = year, y = max_chuva)) +
  ...
  scale_x_continuous(
    breaks = seq(
      floor(min(max_ano$year) / 20) * 20,
      ceiling(max(max_ano$year) / 20) * 20,
      by = 20
    )
  ) +
  ...
  scale_y_continuous(
    breaks = seq(
      0,
      ceiling(max(max_ano$max_chuva) / 100) * 100,
      by = 100
    )
  ) +
  
  labs(
    title = "Máximas chuvas diárias por ano - Brasil",
    x = "Ano",
    y = "Chuva máxima diária (mm)"
  ) +
...
cat(
  "REGIÕES HIDROGRÁFICAS ENCONTRADAS\n"
)
...
# FUNÇÃO PARA ANALISAR UMA RH


analisar_RH <- function(nome_RH) {
  ...
  cat(
    "Estações:",
    n_distinct(dados_RH$gauge_code),
    "\n"
  )
...
cat(
  ...
  n_distinct(
    resultado_final$regiao_hidrografica
  ),
  "\n"
)
...
cat(
  "MTFPW aplicado:",
  sum(
    resultado_final$MTFPW_aplicado,
    na.rm = TRUE
  ),
  "\n"
)
...
cat(
  ...
  sum(
    !resultado_final$autocorrelacao,
    na.rm = TRUE
  ),
  "\n"
)
...
# 10. CRESCENTES E DECRESCENTES APÓS FDR
...
dados_grafico <- tibble(
  ...
  Significativa = c(
    sum(resultado_final$sig_MK, na.rm = TRUE),
    sum(resultado_final$sig_MTFPW, na.rm = TRUE),
    sum(resultado_final$sig_FDR, na.rm = TRUE)
  )
) %>%
  
  mutate(
    Total = nrow(resultado_final),
...
#salvar
ggsave(
  filename = "grafico_sign_metodos_RH.png",
...
coords_estacoes <- dados_maximo_diario %>%
...
resultado_mapa <- resultado_final %>%
...
cat(
  ...
  n_distinct(resultado_mapa$gauge_code),
  "\n"
)
...
cat(
  ...
  n_distinct(
    resultado_mapa$gauge_code[
      !is.na(resultado_mapa$lat) &
        !is.na(resultado_mapa$long)
    ]
  ),
  "\n"
)
