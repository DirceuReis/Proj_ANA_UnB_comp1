...
imax.wateryear <- function(data, durations, start_month = 1, which.mon = 1:12,
  ...
  imax.gauge <- pbapply::pblapply(gauges, function(gauge){
    ...
    if(!is.data.frame(data.gauge)) stop("Elemento ", gauge, " deveria ser data.frame.")
    if(sum(is.element(names[1:2], names(data.gauge))) != 2) stop("Estação ", gauge, " não contém ", names[1], " e ", names[2], ".")
    
    start.month <- get_start_month(gauge)
    if(!is.finite(start.month) || start.month < 1 || start.month > 12) stop("start_month inválido para ", gauge, ".")
    
    data.gauge <- data.gauge[order(data.gauge[[names[1]]]), , drop=FALSE]
    data.gauge <- data.gauge[!is.na(data.gauge[[names[1]]]), , drop=FALSE]
    ...
    if(!is.finite(d.base.hr) || d.base.hr <= 0) stop("Resolução temporal inválida em ", gauge, ".")
    
    ratio <- durations/d.base.hr
    if(any(abs(ratio-round(ratio)) > 1e-8)) stop("Há duração não múltipla da resolução base de ", d.base.hr*60, " min em ", gauge, ".")
    
    shift <- ifelse(start.month > 1 & mon.all >= start.month, 1L, 0L) # se mês atual >= mes inicio ano hidro, +1 (vai p/ proximo ano)
    ...
    # Controle de qualidade de cada ano
    year.qc <- lapply(years, function(yr){
      
      # Definir início e fim do ano hidrológico
      if(start.month == 1){
      ...
      # Nro. observacoes esperadas baseado no time_step (d.base.hr)
      n.expected <- round(as.numeric(difftime(wy.end, wy.start, units="secs")) / (d.base.hr*3600))
    ...
    imax.duration <- lapply(durations, function(d){
      ...
      k <- as.integer(round(d/d.base.hr)) # quantas vezes 'd.base.hr' cabe dentro de 'd'
      
      # Rodar na.rm = TRUE (retornar resultado da soma mesmo que haja NA's na janela)
      depth.agg <- RcppRoll::roll_sum(
        x = depths, n = k, na.rm = TRUE,
        fill=NA_real_, align="left"
      )
      
      # Rodar na.rm = FALSE (retornar NA quando houver NA na janela)
      na.window <- RcppRoll::roll_sum(
        x=as.integer(is.na(depths)), n=k,
        fill=NA_real_, align="left"
      )
      
      idx.ini <- seq_along(dates) # início da janela
      idx.fim <- idx.ini + k - 1L # fim da janela
      ...
      continuous.window[valid.window] <- abs(span.h[valid.window] - d.base.hr*(k-1L)) < 1e-8
      same.wateryear.window <- rep(FALSE, length(dates))
      same.wateryear.window[valid.window] <- wateryear[idx.ini[valid.window]] == wateryear[idx.fim[valid.window]] # ano da janela é o mesmo
      depth.agg[all.na.window | !continuous.window | !same.wateryear.window] <- NA_real_
      ...
      imax.year <- lapply(years, function(yr){
        ...
        if(isTRUE(apply_qc) && !isTRUE(q$qc_year)) return(NULL)
      ...
      data.table::rbindlist(imax.year, use.names=TRUE, fill=TRUE)

    })
    
    imax.duration <- imax.duration[!vapply(imax.duration, is.null, logical(1))]
    
    if(!length(imax.duration)) return(NULL)
    
    data.table::rbindlist(imax.duration, use.names=TRUE, fill=TRUE)
  ...
  imax.gauge <- imax.gauge[!vapply(imax.gauge, is.null, logical(1))]
  
  if(!length(imax.gauge)) return(data.frame())
  
  result <- data.table::rbindlist(imax.gauge, use.names=TRUE, fill=TRUE)
