...
#
#
# Esse script é o mesmo que está em "Relatório 2/Scripts_Maximas/12_subdiaria_imax.R",
# mas aqui não são aplicados controles de qualidade p/ análise feita em "visualizacao/analise_qc.R"
#
# =============================================================================
...
string <- paste0(
  "Intensidades máximas anuais (BR-SDR) --- dados subdiários\n",
  "Fontes:\n",
  "- Ano hidrológico: '", DIR_ANO_HIDRO, "'\n",
  "- Séries contínuas: '", DIR_FLUXO, "'\n",
  "- Percentual máximo de falhas anuais (QC): ", NA_ACCEPT_HYDRO*100, "%\n",
  "- Durações [h]: ", paste(DURATIONS_HR, collapse = ", ")
)

cat(string, file = file.path(DIR_DF, "about.txt"))
...
F_INI <- achar_rds("df_subdiario_ano_hidro_uniplu.rds") # arquivo c/ início do ano hidrológico
...
inv <- readRDS(F_INV) %>% mutate(gauge_code=trimws(as.character(gauge_code))) # table_info
...
for(uf in ufs){
  ...
  n_distinct(meta$gauge_code) # p/ RS 368 estações

  # Testar somente RS
  raw <- arrow::read_parquet(file.path(DIR_FLUXO, "resultados/subdiarios/binded_raw",  paste0(tolower(uf), "_subdaily_data.pqt")))
  raw <- raw |> filter(time_step != 1440) # algumas estações no BR-SDR na verdade são pluviômetros
  raw <- raw |> select(gauge_code, rain_mm, datetime) # remover 'time_step' e 'responsible' pq no da Larissa não tem
  n_distinct(raw$gauge_code) # p/ RS 385 estações
  ...
  message("[",uf,"] linhas=",format(nrow(raw),big.mark="."), " | postos=",n_distinct(raw$gauge_code))
  print(raw %>% distinct(gauge_code,time_step,responsible) %>% count(responsible,time_step,sort=TRUE), n=Inf)
  ...
  filled <- filled %>% mutate(time_step=as.numeric(time_step)/60) # converte de volta p/ minutos
  message("[",uf,"] resoluções: ", paste(sort(unique(filled$time_step)), collapse=", ")," min")
  ...
  data.by.time <- fun_group_ts(data.ls,ts_name="time_step") # cria grupos c/ estações de mesma resolução
  time.steps <- names(data.by.time)                         # diferentes 'time_step's  em UF
  start_map <- setNames(as.integer(rast_uf$mes_inicio),rast_uf$gauge_code) # definir início ano hidrológico
  
  # -------------------------------------------------------------------------
  # IMAX por resolução
  # -------------------------------------------------------------------------

  # ts <- time.steps[4] # testar p/ ts = 60 min
  # rm(ts)
  
  imax.ls <- lapply(time.steps, function(ts){
    
    current.ts <- data.by.time[[ts]] # subgrupo c/ 'time_step' == ts min
    ds <- as.numeric(ts)/60          # converte ts de min p/ hora
    ratio <- DURATIONS_HR/ds
    valid.durations <- DURATIONS_HR[abs(ratio-round(ratio)) <= 1e-8] # durações q podem ser agregadas de 'ds'
    if(!length(valid.durations)) return(NULL)
    
    message("[",uf,"] ts = ",ts," min | postos = ", length(current.ts), " | durações = ", length(valid.durations))
    
    current.imax <- lapply(current.ts, function(df) df[, c("datetime","rain_mm"), drop = FALSE])
    names(current.imax) <- names(current.ts)
    
    sm <- start_map[names(current.imax)] # extrair o mês de início do ano hidrológico p/ cada estação
    keep <- is.finite(sm)
  ...
  qc_uf <- imax_uf |> distinct(gauge_code, wateryear, start_month, na_prct_yr, complete_wateryear, qc_year)
  
  message(
    "[",uf,"] IMAX=",format(nrow(imax_uf),big.mark="."),
...
F_OUT_PQT <- file.path(DIR_DF,"df_imax_subdiario.pqt")
