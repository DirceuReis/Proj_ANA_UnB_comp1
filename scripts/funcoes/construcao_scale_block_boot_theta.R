...
# Aviso: rodar somente p/ 1º momento pq altera o parâmetro de deslocamento (theta/offset)
...
fun_scale_block_boot <- function(df.imax,                   # data.frame com intensidades máximas anuais para diferentes durações
  ...
  fun.obj <- function(offset, data.matrix, d, mom){
    ...
    if(offset < 0 || offset > 1) return(1e9)
...
arrow::write_parquet(teste, "base/gerados/scale_invariance/teste_scale_boot_theta.pqt")
