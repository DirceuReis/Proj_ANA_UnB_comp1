...
onde $\hat{x}$ é o quantil máximo anual na escala da estação, $F$ é a função de distribuição cumulativa
e $F^{-1}$ é sua inversa. $j,o,h$ correspondem a cada dado de precipitação observado subdiário ($‘j’$)
($‘o’$) no período histórico-base ($‘h’$), enquanto $‘m’$ permanece como os dados modelados. Os quantis
extraídos de cada par \hat{x}_{j,o,h} e $x_{m,h}$ são igualados para estabelecer uma relação funcional na
forma da Equação (16):
...
onde $m,f$ corresponde aos dados modelados ($‘m’$) no período de projeção futuro ($‘f’$), e $\Delta_m$ é
a mudança relativa. A série subdiária máxima futura projetada ($X_j,f$ )) na escala da estação é
então gerada usando a Equação (16) substituindo $x_{m,h}$ por $\hat{x}_{m,h}$ e multiplicando-a pela
mudança relativa Dm da Equação (18), conforme dado pela Equação (19):
...
Outras variantes mais recentes incluem o *Bias Correction Quantile Mapping* (BCQM), corrige diretamente as funções de distribuição cumulativa (CDFs)

o *Kernel Density Distribution Mapping* (KDDM), utiliza densidades suavizadas por meio de funções kernel

o *Quantile Delta Mapping* (QDM), preserva explicitamente o sinal de mudança climática ao aplicar ajustes aditivos ou multiplicativos às distribuições

e o PresRat (PRAT),estende o QDM incorporando um fator médio de correção adiciona [@Lanzante.2021].
