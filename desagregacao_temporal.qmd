...
As relações Intensidade–Duração–Frequência (IDF) são elemnetos centrais na engenharia de recursos hídricos, pois orientam o dimensionamento seguro e econômico de drenagem, bueiros, pontes e barragens. Em termos simples, uma curva IDF define $i=f(d,T)$, relacionando a intensidade média máxima $i$ para uma dada duração $d$, associada a uma certa frequência de ocorrência (período de retorno $T$, ou probabilidade anual de excedência). É a partir dessas curvas, que se costuma estimar as chamadas chuvas de projeto, isto é, eventos de magnitude e raridade específicas que a infraestrutura a ser projetada deve suportar [@Vyver.2018].

É útil distinguir IDF (intensidades), DDF (lâmina–duração–frequência) e hietogramas de projeto. As curvas IDF/DDF fornecem valores médios por duração, mas não especificam o padrão temporal do evento chuvoso, que geralmente é obtido por desagregação ou por perfis de projeto. Na prática, as IDFs são frequentemente derivadas com base numa distribuição teórica de probabilidades ajustada aos dados observados, sendo a distribuição Generalizada de Valores Extremos (GEV) ou a distribuição de Gumbel sob hipótese de estacionariedade bastante populares [@Baldassarre.2006; @Courty.2019; @Yeo.2021], embora outras distribuições sejam também empreadas, como a log-Pearson Tipo 3 ou a Generalizada de Pareto, conforme discutido em outrs seçoes deste relatório.

Um desafio recorrente em estudos desta natureza é a assimetria entre a demanda e a oferta de informações na escala temporal desejada. Projetos podem exigir intensidades nas escalas subdiárias (minutos–horas), como no caso de dimensionamento de elementos contituintes de drenagem urbana, enquanto grande parte das observações históricas disponíveis para análise está na escala diária. Além disso, as redes pluviográficas costumam ser esparsas e com séries históricas curtas, o que dificulta a adequada caracterização das intensidade de chuva para as diferentes duações, resultando em estimativas com alto grau e incerteza, podendo incorrer em subdimensionamento das estruturas ou sobrecustos.

A desagregação temporal, descrita aqui, é a principal resposta ao problema entre a oferta e demanda de informaçoes na escala desejada. O objetivo não é apenas “quebrar” um total diário em partes, mas reconstruir uma dinâmica subdiária plausível, preservando: (i) a massa diária (quando aplicável), (ii) a intermitência (sequência seco–úmido), (iii) a variabilidade intradiária e (iv) as propriedades de extremos. Em outras palavras, busca-se gerar séries subdiárias estatisticamente consistentes com a observação e adequadas ao uso em modelos hidrológicos.

Para organizar o vasto campo de metodologias, esta revisão as classifica em cinco famílias principais, seguindo uma progressão lógica que parte de abordagens empíricas diretas em direção a modelos estocásticos e estatísticos cada vez mais abrangentes. A classificação inicia-se com (i) Fórmulas Empíricas e Regionais, que forma a base histórica e prática da engenharia. Em seguida, avança para os (ii) Modelos de Processos Pontuais, que buscam simular a microestrutura geradora da chuva. Uma perspectiva alternativa é apresentada pelos (iii) Modelos Baseados em Invariância de Escala, que exploram simetrias estatísticas fundamentais do processo chuvoso. A revisão aborda também as (iv) Técnicas Não Paramétricas, que se libertam de premissas distributivas por meio de reamostragem, e finaliza com a (v) Modelagem Hierárquica de Extremos, que representa o arcabouço mais moderno para a síntese de informação e a quantificação da incerteza.
...
A partir desses pontos, derivam-se curvas IDF/DDF completas usando a base diária [@cetesb1979-drenagem; @Abreu.2022]. No contexto brasileiro, os coeficientes de desagregação mais amplamente aplicados são originalmente estabelecidos pela CETESB [-@cetesb1979-drenagem]. Eles são obtidos a partir da média de registros subdiários provenientes de pluviógrafos instalados em diversas localidades do país e, por isso, acabam sendo considerados como coeficientes gerais para todo o território nacional.
...
via método de Gauss–Newton sobre os pontos gerados pelos coeficientes. Para a espacialização do $CD_{\mathrm{specific}}$ aplicam a técnica de interpolação por Ponderação do Inverso da Distância (*Inverse Distance Weighting – IDW*). O Atlas Pluviométrico do Brasil [@cprm2013-metodologia-idf] adota uma abordagem paramétrica mais elaborada, utilizando uma relação log-linear entre a precipitação acumulada $P$ (mm) e a duração $d$, cujos coeficientes dependem do período de retorno e $T$ (anos):
...
### Isozonas de desagregação

A metodologia de isozonas é uma abordagem de regionalização que se baseia no princípio de que áreas climaticamente semelhantes exibem comportamento homogêneo em relação às chuvas intensas. Proposto por Torrico [-@torrico1974-praticas] para o Brasil, o conceito foi aplicado e revisitado por Basso et al. [-@Basso.2016], que utilizaram um conjunto mais amplo para redefinir as zonas homogêneas e os coeficientes de desagregação regionais. No processo de rezoneamento, as intensidades e as relações entre durações obtidas a partir das IDF atuais são comparadas às estimadas por Torrico [-@torrico1974-praticas] com ênfase em $r_{1(h)},24h$, apoiando-se em mapas climáticos nacionais para áreas com baixa densidade de dados e verificando a homogeneidade intra-zona por estatísticas descritivas. O resultado é um conjunto de novas isozonas e coeficientes regionais de desagregação $r_{d(h)},24h$, para $T=10$ anos.
...
Modelos de processos pontuais oferecem um quadro estocástico robusto para a desagregação temporal em passo fino, com estrutura de aglomeração, parâmetros fisicamente interpretáveis e boa capacidade de reproduzir estatísticas em múltiplas escalas, e constituem a base de ferramentas consolidadas como HYETOS e HyetosMinute [@Qin.2024], mas seus altos requisitos de dados sub-horários, calibração multivariada complexa e custo computacional [@Vorobevskii.2023] justificam uma abordagem alternativa. Dado o foco do estudo em quantificar quantis de projeto de forma parcimoniosa, adotaremos um arcabouço coerente com a EVT, tratando os processos pontuais como referência conceitual e pesquisa futura.
...
onde $m_h(\lambda)$ é o momento de ordem $h$ da precipitação acumulada, $a_h$ é o intercepto logarítmico na lei de escala simples e $\theta_h$ é o expoente de escala. No multiescalamento (sentido amplo) a relação geral passa a ser
...
Burlando & Rosso [-@Burlando.1996] constroem um arcabouço para curvas lâmina–duração–frequência (DDF) a partir da hipótese de invariância de escala. A ideia central é estimar conjuntamente (i) o expoente de escala $\eta$, que governa a média e (ii) uma função de dissipação $\psi_\zeta$, que descreve o crescimento dos momentos de ordem $\zeta$ (multiescala), primeiro em um esquema livre de distribuição e, em seguida, com uma modelagem lognormal (LN2) parcimoniosa. No caso de escala simples (sentido amplo), entre um corte interno $D_i$ e as durações $D$ de interesse, a média e a variância de máximos anuais obedecem leis de potência de $D$. Disso resulta que o coeficiente de variação e, de forma mais ampla, os momentos adimensionais (assimetria e curtose) não dependem de $D$. Esse resultado leva a uma família invariante à escala de DDF:
...
No limite da escala simples, recupera-se $\eta_1=\eta_2=\eta$ e a forma separável usual. A estimação é feita em um framework Bayesiano, com critérios de comparação que indicam melhor desempenho e controle de complexidade (sem sobreajuste) em relação a formulação de escala simples quando os dados sugerem comportamento multiescala.
...
Na literatura de desagregação temporal, destacam-se abordagens baseadas em relações de escala acopladas à GEV (scaling-GEV), úteis sempre que se deseja transferir informação entre durações sem depender do contexto climático específico. @Requena.2021 propõem um modelo que segue a formulação da escala simples com a distribuição GEV. Como as leis de escala podem mudar entre mecanismos de chuva de curta e longa duração, o trabalho admite multi-escalação por trechos: identifica-se um ponto de quebra no gráfico log–log e estima-se $\eta$ separadamente antes/depois da quebra, escolhendo o ponto que minimiza os resíduos de regressões lineares nos dois trechos (usando os três primeiros momentos, suficientes para parametrizar a GEV). A presença de quebra implica dois expoentes $\eta$ distintos e conduz a uma desagregação *piecewise* coerente entre durações. Já Cannon & Innocenti [-@Cannon.2018] mostram um pipeline prático de ajuste scaling-GEV para séries com resoluções mistas (diária/subdiária) e amostras relativamente curtas: ajusta-se uma GEV sob escala simples, com inferência Bayesiana (MCMC) e seleção de mudanças via *Bayesian False Discovery Rate* para decidir, de forma parcimoniosa, quando alterações nos parâmetros/relacionamentos de escala são de fato suportadas pelos dados. O desempenho é verificado contra IDFs observadas e grades observacionais de alta resolução, reforçando procedimentos de validação cruzada entre durações. Em conjunto, esses estudos oferecem um arcabouço operacional para transferir quantis/extremos entre escalas temporais via leis de escala com GEV, que é exatamente o foco desta seção sobre desagregação temporal.
