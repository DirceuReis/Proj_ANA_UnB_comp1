...
As relações Intensidade–Duração–Frequência (IDF) são elemnetos centrais na engenharia de recursos hídricos, pois orientam o dimensionamento seguro e econômico de drenagem, bueiros, pontes e barragens. Em termos simples, uma curva IDF define $i=f(d,T)$, relacionando a intensidade média máxima $i$ para uma dada duração $d$, associada a uma certa frequência de ocorr^enca (período de retorno $T$, ou probabilidade anual de excedência). É a partir dessas curvas, que se costuma estmar as camadas chuvas de projeto, isto é, eventos de magnitude e raridade específicas que a infraestrutura a ser projetada deve suportar [@Vyver.2018].
