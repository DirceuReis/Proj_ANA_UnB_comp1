...
bibliography: referencias/refs_guidelines.bib
---
...
Aqui serão descritas as metodologias adotadas para a estimativa de chuvas extremas na Inglaterra, Austrália, Canadá e Estados Unidos. Os estudos foram desenvolvidos em diferentes contextos, e consolidaram diretrizes para a análise desses eventos em cada país. As metodologias contemplam desde a seleção e o tratamento de dados pluviométricos até a derivação de modelos paramétricos, integrando técnicas estatísticas e métodos de interpolação espacial.

**1. INGLATERRA**

Documento: ***Flood Estimation Handbook (FEH). Vol 2: Rainfall Frequency estimation*** [@Faulkner.1999].
...
A base de dados utilizada compreendeu:

-   6106 estações com registros diários, cada uma com pelo menos 10 anos de dados;

-   375 estações com registros subdiários (horários), provenientes de pluviômetros automáticos de balde basculante e de sifão inclinável, cada uma com no mínimo 9 anos de séries de máximos anuais.
...
**1.1.2 Série de máximos anuais e índice de referência**

A partir dos registros, foram extraídas as séries de máximos anuais (AM) em diferentes durações (1h, 2h, 6, 12, 1d, 2, 4d e 8d). Essas séries constituíram a base para a análise estatística.
...
**1.1.4 Derivação das curvas de crescimento (*Growth Curves*)**
...
O método utilizou a divisão dos dados em faixas de retorno (y-slices) e ajusta segmentos lineares por mínimos quadrados, aplicando suavização para garantir consistência entre diferentes períodos de retorno.
...
A incerteza foi avaliada por meio de reamostragem *bootstrap*, gerando a quantificação dos intervalos de confiança para as curvas resultantes.

**1.1.5 Construção do Modelo DDF (*Depth-Duration-Frequency*)**

A integração do RMED com as curvas de crescimento deu origem a um modelo empírico-paramétrico DDF, que relaciona profundidade de chuva, duração e frequência, (Equação 3).
...
O modelo DDF foi parametrizado por 6 parâmetros (c, d₁, d₂, d₃, e, f) ajustados separadamente para cada ponto da grade de 1 km.
...
**1.1.7 Atualizações da Metodologia**

Documentos: *Reservoir Safety – Long Return Period Rainfall Volume 1 Technical Report (Part 1 and 2)* [@Stewart.2013a; @Stewart.2013b]

Em 2013, a *Environment Agency* e o *Departament for Envionment Food and Rural Affairs* publicaram os volumes 1 e 2 do referido documento que possuem atualizações para a metodologia. Assim, destacam-se as seguintes mudanças:

-   Expansão no tamanho da base de dados de 375 para 968 estações horárias;
...
-   No FEH, Vol 2 a dependência espacial era tratada de forma simplificada e constante, ou seja, assumia-se um certo grau de correlação espacial entre pluviômetros, independentemente da raridade do evento. Nos relatórios *Long Return Period Rainfall*, a dependência entre estações varia com o período de retorno: Para eventos comuns (baixo período de retorno) há uma forte dependência, considerando que chuvas intensas costumam ocorrer em várias estações próximas ao mesmo tempo; para eventos raros (período de retorno alto) há menor dependência, considerando que eventos muito extremos são mais localizados e não atingem grandes áreas com a mesma intensidade;
...
Documento: ***Australian Rainfall and Runoff, 2019*** [@ARR.2019].

**2.1 Descrição da Metodologia**

A metodologia para estimativa das chuvas de projeto na Austrália, é descrita no contexto das revisões do *Australian Rainfall and Runoff* [@ARR.2019]. A abordagem adotada baseia-se em um conjunto integrado de etapas, que incluem a criação de uma base de dados nacional de precipitação, o controle de qualidade dos registros, a extração de séries de valores extremos, a análise de frequência, a regionalização e a interpolação espacial.

**2.1.1 Construção de uma base de dados nacional**

Foram utilizadas estações pluviométricas diárias e subdiárias, operadas pelo *Bureau of Meteorology* e por diversas outras instituições. O conjunto incluiu aproximadamente 8.074 estações de leitura diária e 2.280 estações de registro subdiário, considerando todos os dados disponíveis até 2012. Para serem utilizadas, as estações deveriam cumprir o critério de comprimento de série (≥30 anos para diárias; \>8 anos para subdiárias). Foi adotado um controle de qualidade automático e manual das séries, incluindo checagem de metadados, remoção de erros grosseiros e ajustes de séries acumuladas. Dois métodos foram aplicados para realização do teste de estacionariedade:

(i) análise de tendências nos máximos anuais por estações, e

(ii) abordagem regional de médias por área. A partir dos resultados dos testes, concluiu-se que era aceitável assumir estacionariedade e usar o período completo de dados.

**2.1.2 Extração das séries de extremos**

A série de máximos anuais (AMS) foi usada para chuvas frequentes e infrequentes (até 1% da AEP) e a série de duração parcial (PDS) foi utilizada para chuvas muito frequentes (Período de retorno \< que 1 ano).

**2.1.3 Análise de frequência**

A distribuição GEV foi selecionada para ajustar a AMS, sendo os parâmetros da distribuição estimados por meio do L-momentos. A distribuição *Generalised Pareto* (GPA) foi adotada somente para eventos da PDS, ajustada também por L-momentos. A *Bayesian Generalised Least Squares Regression* (BGLSR) foi aplicado para os dados subdiários.

**2.1.4 Estimativa de L-momentos (média, L-CV, L-assimetria)**
...
Para as estações subdiárias (≥ 8 anos), os L-momentos foram calculados diretamente da AMS sub-diária. Entretanto, para as estações de leitura diária (≥ 30 anos), onde faltavam registros sub-diários, o BGLSR foi aplicado para predizer os L-momentos das durações menores que 24h, a partir de variáveis explicativas (latitude, longitude, elevação, distância da costa, precipitação média anual etc.). Esses L-momentos estimados foram então usados para definir os parâmetros da GEV sub-diária.
...
**2.1.6 Interpolação espacial**

Os parâmetros da GEV foram interpolados em grade nacional usando o software ANUSPLIN utilizando curvas que consideram latitude, longitude e elevação, sendo a escala de altitude exagerada por um fator de 100, para representar a importância que a altitude tem nos padrões de precipitação. Para disponibilizar resultados em locais não monitorados, os parâmetros regionalizados da GEV foram interpolados por meio do mesmo *software*, aplicando também curvas de suavização em função da latitude, longitude e altitude. Essa etapa produziu superfícies contínuas de parâmetros, permitindo estimar curvas IDF em qualquer ponto do território australiano.
...
*Environmental Modelling & Software*. [@Simonovic.2016].
...
**3.1.1 Curvas de Intensidade-Duração-Frequência**

Inicialmente, destaca-se que foram utilizados dados subdiários de precipitação das estações pluviométricas de diferentes intervalos de tempo (5, 10, 15 e 30 minutos e 1, 2, 6, 12 e 24 horas). A ferramenta IDF_CC adotou a distribuição Gumbel para ajustar a precipitação máxima anual (AMP) em todo o Canadá. O método dos momentos foi utilizado para estimar os parâmetros da distribuição Gumbel.

**3.1.2 *Downscaling* espacial e temporal da precipitação**

Para atualizar as curvas IDF foi adotado o método *Equidistant Quantile Matching* (EQM), desenvolvido por @Srivastav.2014. Esse método realiza o *downscaling temporal* (a distribuição das mudanças entre o período de tempo projetado e o período de linha de base), além do *downscaling espacial* da precipitação máxima anual (AMP derivada dos dados do GCM e dos dados subdiários observados).

O EQM foi aplicado diretamente à AMP para estabelecer as relações estatísticas entre as AMPs dos dados de precipitação gerados pelo GCM e os dados observados (históricos) subdiários, ao invés de usar registros completos de precipitação diária.
...
**3.1.3 Descrição do método EQM**

A descrição do método descrita a seguir é a mesma encontrada no anexo do artigo de @Simonovic.2016.
...
Quando o usuário opta por criar a IDF de uma região, o gráfico de IDF é gerado a partir de uma distribuição GEV ou Gumbel, a seu critério, sendo possível acessar a tabela com os dados gerados e as equações de interpolação (ou seja, os coeficientes A, B, C e t₀ para o cálculo da intensidade). No caso da IDF sob mudanças climáticas, o usuário deve escolher um intervalo de tempo de, no mínimo, 30 anos, entre 2015 e 2100, e selecionar entre as distribuições GEV e Gumbel. Em seguida, é necessário escolher o modelo climático que será utilizado, entre o CMIP6 e o CMIP5.

Para cada modelo, é possível selecionar o conjunto de multimodelos (selecionando todos ou personalizando). Após essas escolhas, basta clicar no botão "Continuar" para acessar as curvas IDF's, tabelas, equações de interpolação e o boxplot das incertezas para os três cenários de emissão e concentração (SSP1.26, SSP2.45 e SSP5.85).
...
Documento: ***NOAA Atlas 15 Pilot: Technical Report*** [@Perica.2024].

O relatório é dividido em duas partes: 

(1) Volume 1 que fornece estimativas de Frequência da Precipitação (PF) que levam em conta tendências temporais em observações históricas; 

(2) Volume 2 que fornece estimativas de PF projetadas para o futuro, sob diferentes cenários de emissões de carbono, com base em projeções de modelos climáticos.

**4.1 Metodologia do Volume 1 - Estimativas históricas não estacionárias**

**4.1.1 Conjunto de dados relevantes**

O relatório utilizou dados produzidos e/ou adquiridos a partir do Atlas 14 - Vol. 12 [@Bonnin.2004]:
...
**4.1.2 Distribuição estatística**
...
A função de distribuição de probabilidade GEV não estacionária utilizada é descrita a seguir:
...
onde $\mu$, $\sigma$, e $\xi$, são parâmetros de localização, escala e forma, respectivamente, 𝑥 é uma coordenada espacial e 𝑡 representa o tempo (ano). Para cada região os parâmetros foram definidos da seguinte forma:
...
Foi aplicado um esquema de pesos com função Kernel para atribuir relevância a cada estação regional. Onde 0 indica nenhuma semelhança entre as estações e 1 indica que estão geograficamente próximas, e que possuem semelhanças nos atributos da precipitação.

**4.1.4 Covariáveis espaciais e temporais**

As covariáveis espaciais e temporais foram utilizadas para calcular os parâmetros de localização e escala da distribuição GEV. Foram testadas várias covariáveis espacias em grade (elevação, MAP, MAM, dentre outras). O parâmetro MAM teve o melhor desempenho em todas as durações. Para o Volume 1, o GWL foi selecionado como a covariável temporal, definida como a anomalia da temperatura global média móvel de 30 anos em relação ao período pré-industrial (1851 – 1900).

**4.1.5 Interpolação para grade**
...
**4.2 Metodologia do Volume 2 - Projeções futuras com modelos climáticos**

**4.2.1 Conjunto de dados relevantes**
...
**4.2.2 Fatores de ajuste**

Os parâmetros de distribuição GEV para o Volume 2 foram estimados usando valores AMS em grade extraídos do LOCA2 e do STAR-ESDM. Os parâmetros de localização, escala e forma, que foram calculados de maneira semelhante ao Volume 1, foram, portanto, identificados para cada célula da grade dentro do domínio e para cada um dos 32 conjuntos para durações diárias e mais longas durante o período de 1950 a 2100. As distribuições GEV resultantes foram usadas para gerar estimativas de frequência de precipitação em resoluções de células de grade nativas para cada modelo estatístico. Assim, enquanto as estimativas do Volume 1 se originaramm em locais de estações, com um valor por duração e AEP, as estimativas do Volume 2 se originam em uma grade, com 32 valores (um de cada conjunto de modelos) por duração e AEP. Uma vez obtidas as estimativas absolutas de frequência de precipitação para cada GWL (1,5 - 5 °C), suas mudanças, em relação ao ano base de 2023, foram computadas. As estimativas de frequência de precipitação foram geradas para o presente (2023) e, em seguida, novamente para um momento futuro quando os GCMs atingirem 3 °C. Para cada nível de aquecimento, foi calculado:
...
Em seguida, espacialmente, os fatores foram suavizados por meio de um filtro gaussiano para reduzir ruídos locais e, posteriormente, foram interpolados em grade contínua com interpolação cúbica monotônica, assegurando transições suaves e consistentes entre células vizinhas.
...
O termo multiplicativo foi baseado em um fator de ajuste que expressa (em unidades percentuais) a mudança relativa no futuro a partir da linha de base do PFVOL1, que representa o ano de 2023. Assim, os valores finais do PFVOL2 representaramm estimativas futuras de precipitação para as mesmas durações e AEPs do PFVOL1, mas também são funções da temperatura global (estrutura do GWL) e do tempo.
