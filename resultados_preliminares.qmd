...
Adotou-se um mínimo de 8 anos de dados por estação, para garantir (i) um número mínimo de observações para a estimação da GEV em cada posto e (ii) um número suficientemente grande de estações na amostra, evitando ao mesmo tempo um custo computacional excessivo no ajuste do modelo hierárquico multisite.

Em cada combinação estação–ano, calculou-se a proporção de dados faltantes de imax e foram removidos os anos com mais de 80% de NA (prop_na > 0,8). Dessa forma, apenas anos com cobertura mínima razoável são utilizados na estimação das distribuições extremas.
...
dur_keep_h <- seq(1,24,1)
min_years  <- 8

dados_amax <- dados_agregados %>%
  mutate(
    year = as.integer(year),
    d    = as.numeric(d),
    imax = as.numeric(imax)
  ) %>%
  group_by(gauge_code, d, year) %>%
  slice_max(imax, n = 1, with_ties = FALSE) %>%
  ungroup()

na_lim <- 0.2
dados_amax_qc <- dados_amax %>%
  filter(is.na(na_prct) | na_prct <= na_lim)

dados_amax_qc <- dados_amax_qc %>%
  filter(d %in% dur_keep_h)

years_stats <- dados_amax_qc %>%
  group_by(gauge_code, d) %>%
  summarise(
    n_years_valid = sum(!is.na(imax)),
    n_years_total = n_distinct(year),
    .groups = "drop"
  )

dados_filtrado <- dados_amax_qc %>%
  left_join(years_stats, by = c("gauge_code", "d")) %>%
  filter(n_years_valid >= min_years) %>%  
  filter(!is.na(imax)) %>%
  select(gauge_code, year, d, imax, date, na_prct, mon_filter, n_max, n_years_valid, n_years_total)

years_stats %>%
  filter(n_years_valid > min_years) %>%
  count(d, name = "n_gauges_pass")

dados_modelo <- dados_filtrado %>%
  mutate(
    duration_h = as.integer(d),
    year = as.integer(year),
    imax = as.numeric(imax)
  ) %>%
  filter(duration_h %in% dur_keep_h) %>%
  group_by(gauge_code, duration_h) %>%
  mutate(n_years_valid = n_distinct(year[!is.na(imax)])) %>%
  ungroup() %>%
  filter(n_years_valid >= min_years) %>%
  filter(!is.na(imax)) %>% 
  select(gauge_code, year, duration_h, imax)
```
...
ggplot(summary_dur, aes(x = duration_h)) +
...
ggplot(ajustes_df, aes(x = log(duration_h), y = mu)) +
...
df <- dados_modelo %>%
  mutate(
    gauge = as.integer(factor(gauge_code)),
    dur_factor = factor(duration_h, levels = sort(unique(duration_h))),
    dur_id = as.integer(dur_factor)
  ) %>%
  arrange(gauge, dur_id, year)

y <- df$imax
gauge_id <- df$gauge
duration_id <- df$dur_id
D <- sort(unique(df$duration_h)) # horas

stan_data <- list(
  N_obs = length(y),
  y = as.numeric(y),
  n_gauge = length(unique(gauge_id)),
  K = length(D),
  gauge_id = as.integer(gauge_id),
  duration_id = as.integer(duration_id),
  D = as.numeric(D),
  D0 = 24
)
...
# Checagens
stopifnot(
  all(stan_data$gauge_id >= 1),
  all(stan_data$duration_id >= 1),
  all(stan_data$gauge_id <= stan_data$n_gauge),
  all(stan_data$duration_id <= stan_data$K),
  identical(stan_data$D, sort(unique(stan_data$D)))
)

# Diagnóstico rápido do que entrou
cat("Gauges:", stan_data$n_gauge, "\n")
cat("Durations (h):", paste(stan_data$D, collapse = ", "), "\n")
...
fit <- readRDS("C:\\Users\\laris\\OneDrive\\3. UFC\\UFC - 2026\\Art. 01_BHM\\fit_pooling_partial_8anos.rds")
...
check_all_diagnostics(fit) \[1\] "n_eff / iter looks reasonable for all parameters" \[1\] "Rhat looks reasonable for all parameters" \[1\] "0 of 1000 iterations ended with a divergence (0%)" \[1\] "0 of 1000 iterations saturated the maximum tree depth of 15 (0%)" \[1\] "E-FMI indicated no pathological behavior"
...
join_with_ordinary <- function(df_post, ajustes_df, col_ordinary, rename_to) {
  ajustes_sel <- ajustes_df %>%
    rename(Estacao = station, Duration = duration_h) %>%
...
df_years <- anos_por_estacao %>%
  mutate(
    grupo = case_when(
      n_years %in% c(17,14) ~ "Longas (17,14 anos)",
...
p_xi_longas <- plot_param(
  df_xi_comb %>% filter(grupo == "Longas (17,14 anos)"),
  "xi_gev",
  expression(xi),
  "xi – Estações com séries longas (17-14 anos)",
  ncol = 5
)
```
...
## Posterior dos parâmetros por estação ($\eta$, θ e α)
