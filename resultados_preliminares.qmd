...
```{r plotando-por-duração-da-série-parte-1}
...
p_mu_longas <- plot_param(
  df_mu_comb %>% filter(grupo == "Longas (18,15,14 anos)"),
  "mu_gev",
  expression(mu),
...
## Posterior dos parâmetros por estação (H, θ e α)
Nesta seção analisamos a distribuição posterior dos parâmetros **H**, **θ** e
**α** do modelo hierárquico. Esses parâmetros controlam, respectivamente:

- **H**: o expoente da relação intensidade–duração, determinando a inclinação
  das curvas IDF para cada estação;
- **θ**: o deslocamento da duração, que ajusta o comportamento da curva para
  durações curtas;
- **α**: o nível da curva de intensidade na duração de referência.

O objetivo desta análise é comparar o comportamento posterior desses
parâmetros entre estações, levando em conta o comprimento da série de
cada uma. Como diferentes estações possuem diferentes números de anos de dados
(n_anos), agrupamos as estações conforme seu comprimento:
...
Geramos então os gráficos posteriores de H, θ e α para cada grupo, evitando
misturar estações com volumes muito distintos de informação.

A estação SBPA apresenta um comportamento bem distinto das demais com o valor alto de H, e baixo alpha.

```{r grafico-posteriori}
...
extract_param_long <- function(draws, par_regex, value_name) {
draws %>%
select(matches(par_regex)) %>%
mutate(.draw = row_number()) %>%
tidyr::pivot_longer(-.draw, names_to = "name", values_to = value_name) %>%
mutate(station_id = as.integer(stringr::str_extract(name, "\\d+"))) %>%
select(.draw, station_id, !!value_name)
}

library(dplyr)

H_long     <- extract_param_long(draws, "^H\\[",     "H")
theta_long <- extract_param_long(draws, "^theta\\[", "theta")
alpha_long <- extract_param_long(draws, "^alpha\\[", "alpha")

posterior_df <- H_long %>%
inner_join(theta_long, by = c(".draw", "station_id")) %>%
inner_join(alpha_long, by = c(".draw", "station_id")) %>%
mutate(station = factor(station_labels[station_id],
levels = station_labels))
...
## Mapas interpolados dos parâmetros H e θ com identificação das estações

Os parâmetros **H** e **θ** do modelo de escala intensidade–duração foram estimados para cada estação pluviométrica. Para visualizar sua variação espacial no estado do Rio Grande do Sul, realizamos uma interpolação espacial do tipo *Inverse Distance Weighting* (IDW) sobre uma grade regular, gerando campos contínuos de H e θ.
...
x <- readRDS("C:\\Users\\laris\\Downloads\\rs_info_table.rds")

s_par <- rstan::summary(fit, pars = c("H", "theta"))$summary
...
H_grid     <- interpolar_campo(stations_Htheta_sf, grid_sf, "H") %>% mutate(param = "H")
theta_grid <- interpolar_campo(stations_Htheta_sf, grid_sf, "theta") %>% mutate(param = "theta")
